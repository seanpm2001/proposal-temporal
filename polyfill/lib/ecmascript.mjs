...
export const ES = ObjectAssign({}, ES2022, {
  ...
  ToRelativeTemporalObject: (options) => {
    ...
    if (ES.Type(relativeTo) === 'Object') {
      ...
      if (ES.IsTemporalDate(relativeTo)) return { relativeTo };
      if (ES.IsTemporalDateTime(relativeTo)) return { relativeTo: ES.TemporalDateTimeToDate(relativeTo) };
    ...
    if (timeZone === undefined) {
      return { relativeTo: ES.CreateTemporalDate(year, month, day, calendar) };
    }
    ...
    return {
      relativeTo: ES.CreateTemporalZonedDateTime(epochNanoseconds, timeZone, calendar),
      timeZoneRec
    };
  },
  ...
  ToTemporalDate: (item, options) => {
    ...
    if (ES.Type(item) === 'Object') {
      ...
      if (ES.IsTemporalZonedDateTime(item)) {
        ...
        item = ES.GetPlainDateTimeFor(timeZoneRec, GetSlot(item, INSTANT), GetSlot(item, CALENDAR));
      }
  ...
  ToTemporalDateTime: (item, options) => {
    ...
    if (ES.Type(item) === 'Object') {
      ...
      if (ES.IsTemporalZonedDateTime(item)) {
        ...
        const timeZoneRec = new MethodRecord(GetSlot(item, TIME_ZONE), ['getOffsetNanosecondsFor']);
        return ES.GetPlainDateTimeFor(timeZoneRec, GetSlot(item, INSTANT), GetSlot(item, CALENDAR));
      }
  ...
  InterpretISODateTimeOffset: (
    ...
    if (offsetBehaviour === 'wall' || offsetOpt === 'ignore') {
      ...
      const instant = ES.GetInstantFor(timeZoneRec, dt, disambiguation);
    ...
    // "prefer" or "reject"
    const possibleInstants = ES.GetPossibleInstantsFor(timeZoneRec.receiver, dt, timeZoneRec.getPossibleInstantsFor);
    for (let index = 0; index < possibleInstants.length; index++) {
      ...
      const candidateOffset = ES.GetOffsetNanosecondsFor(
    ...
    if (offsetOpt === 'reject') {
      ...
      const timeZoneString = ES.IsTemporalTimeZone(timeZoneRec.receiver)
    ...
    if (!timeZoneRec.getOffsetNanosecondsFor) timeZoneRec.lookup('getOffsetNanosecondsFor');
    const instant = ES.DisambiguatePossibleInstants(possibleInstants, timeZoneRec, dt, disambiguation);
    return GetSlot(instant, EPOCHNANOSECONDS);
  },
  ...
  GetPlainDateTimeFor: (timeZoneRec, instant, calendar, precalculatedOffsetNs = undefined) => {
    // Either getOffsetNanosecondsFor must be looked up, or
    // precalculatedOffsetNs should be supplied
    const ns = GetSlot(instant, EPOCHNANOSECONDS);
    const offsetNs =
      precalculatedOffsetNs ??
      ES.GetOffsetNanosecondsFor(timeZoneRec.receiver, instant, timeZoneRec.getOffsetNanosecondsFor);
  ...
  GetInstantFor: (timeZoneRec, dateTime, disambiguation) => {
    ...
    return ES.DisambiguatePossibleInstants(possibleInstants, timeZoneRec, dateTime, disambiguation);
  },
  DisambiguatePossibleInstants: (possibleInstants, timeZoneRec, dateTime, disambiguation) => {
    // getPossibleInstantsFor must be looked up already.
    ...
    const offsetBefore = ES.GetOffsetNanosecondsFor(
      timeZoneRec.receiver,
      dayBefore,
      timeZoneRec.getOffsetNanosecondsFor
    );
    const offsetAfter = ES.GetOffsetNanosecondsFor(timeZoneRec.receiver, dayAfter, timeZoneRec.getOffsetNanosecondsFor);
    ...
    switch (disambiguation) {
      case 'earlier': {
        ...
        return ES.GetPossibleInstantsFor(
          timeZoneRec.receiver,
          earlierPlainDateTime,
          timeZoneRec.getPossibleInstantsFor
        )[0];
      }
      case 'compatible':
      ...
      case 'later': {
        ...
        const possible = ES.GetPossibleInstantsFor(
          timeZoneRec.receiver,
          laterPlainDateTime,
          timeZoneRec.getPossibleInstantsFor
        );
  ...
  BalanceISODate: (year, month, day) => {
    ...
    if (MathAbs(day) > daysIn400YearCycle) {
      const nCycles = MathFloor(day / daysIn400YearCycle);
      year += 400 * nCycles;
  ...
  NanosecondsToDays: (nanoseconds, zonedRelativeTo, timeZoneRec) => {
    // getOffsetNanosecondsFor and getPossibleInstantsFor must be looked up if zonedRelativeTo is given
    const TemporalInstant = GetIntrinsic('%Temporal.Instant%');
    ...
    const dtStart = ES.GetPlainDateTimeFor(timeZoneRec, start, calendar);
    const dtEnd = ES.GetPlainDateTimeFor(timeZoneRec, end, calendar);
    ...
    let relativeResult = ES.AddDaysToZonedDateTime(start, dtStart, timeZoneRec, calendar, days);
    // may disambiguate
    ...
    if (sign === 1) {
      while (days.greater(0) && relativeResult.epochNs.greater(endNs)) {
        days = days.prev();
        relativeResult = ES.AddDaysToZonedDateTime(start, dtStart, timeZoneRec, calendar, days.toJSNumber());
  ...
  BalancePossiblyInfiniteDuration: (
    ...
    if (largestUnit === 'year' || largestUnit === 'month' || largestUnit === 'week' || largestUnit === 'day') {
      ({ days, nanoseconds } = ES.NanosecondsToDays(nanoseconds, zonedRelativeTo, timeZoneRec));
    } else {
      days = 0;
    }
  ...
  UnbalanceDurationRelative: (years, months, weeks, days, largestUnit, plainRelativeTo, calendarRec) => {
    // calendarRec must be present and have looked up:
    // largestUnit month and years !== 0, dateAdd and dateUntil
    // largestUnit week and years !== 0 and months !== 0, dateAdd
    // largestUnit day or smaller, and years, months, weeks !== 0, dateAdd
    const TemporalDuration = GetIntrinsic('%Temporal.Duration%');
      ...
      case 'month':
        {
          ...
          while (!years.abs().isZero()) {
            const newRelativeTo = ES.CalendarDateAdd(
            ...
            const untilResult = ES.CalendarDateUntil(
              calendarRec.receiver,
              plainRelativeTo,
              newRelativeTo,
              untilOptions,
      ...
      case 'week':
        {
          ...
          while (!years.abs().isZero()) {
            let oneYearDays;
            ({ relativeTo: plainRelativeTo, days: oneYearDays } = ES.MoveRelativeDate(
              calendarRec,
              plainRelativeTo,
              oneYear
            ));
          ...
          while (!months.abs().isZero()) {
            let oneMonthDays;
            ({ relativeTo: plainRelativeTo, days: oneMonthDays } = ES.MoveRelativeDate(
              calendarRec,
              plainRelativeTo,
              oneMonth
            ));
      ...
      default:
        {
          ...
          while (!weeks.abs().isZero()) {
            let oneWeekDays;
            ({ relativeTo: plainRelativeTo, days: oneWeekDays } = ES.MoveRelativeDate(
              calendarRec,
              plainRelativeTo,
              oneWeek
            ));
  ...
  BalanceDurationRelative: (years, months, weeks, days, largestUnit, plainRelativeTo, calendarRec) => {
    ...
    switch (largestUnit) {
      case 'year': {
        ...
        ({ relativeTo: newRelativeTo, days: oneYearDays } = ES.MoveRelativeDate(calendarRec, plainRelativeTo, oneYear));
        ...
        newRelativeTo = ES.CalendarDateAdd(
          calendarRec.receiver,
          plainRelativeTo,
          oneYear,
          undefined,
        ...
        const untilOptions = ObjectCreate(null);
        untilOptions.largestUnit = 'month';
        let untilResult = ES.CalendarDateUntil(
          calendarRec.receiver,
          plainRelativeTo,
          newRelativeTo,
          untilOptions,
        ...
        while (months.abs().geq(MathAbs(oneYearMonths))) {
          ...
          const untilOptions = ObjectCreate(null);
          untilOptions.largestUnit = 'month';
          untilResult = ES.CalendarDateUntil(
            calendarRec.receiver,
            plainRelativeTo,
            newRelativeTo,
            untilOptions,
      ...
      case 'week': {
        ...
        ({ relativeTo: newRelativeTo, days: oneWeekDays } = ES.MoveRelativeDate(calendarRec, plainRelativeTo, oneWeek));
  ...
  DifferenceDate: (calendarRec, plainDate1, plainDate2, options) => {
    ...
    return ES.CalendarDateUntil(calendarRec.receiver, plainDate1, plainDate2, options, calendarRec.dateUntil);
  },
  DifferenceISODateTime: (
    ...
    let { years, months, weeks, days } = ES.DifferenceDate(calendarRec, date1, date2, untilOptions);
  ...
  DifferenceZonedDateTime: (ns1, ns2, timeZoneRec, calendar, largestUnit, options, calendarRec) => {
    ...
    const intermediate = ES.CreateTemporalZonedDateTime(intermediateNs, timeZoneRec.receiver, calendar);
    ({ nanoseconds: timeRemainderNs, days } = ES.NanosecondsToDays(timeRemainderNs, intermediate, timeZoneRec));
  ...
  DifferenceTemporalPlainDate: (operation, plainDate, other, options) => {
    ...
    let { years, months, weeks, days } = ES.DifferenceDate(calendarRec, plainDate, other, resolvedOptions);
  ...
  DifferenceTemporalPlainYearMonth: (operation, yearMonth, other, options) => {
    ...
    thisFields.day = 1;
    const thisDate = ES.CalendarDateFromFields(calendarRec.receiver, thisFields, undefined, calendarRec.dateFromFields);
    const otherFields = ES.PrepareTemporalFields(other, fieldNames, []);
    otherFields.day = 1;
    const otherDate = ES.CalendarDateFromFields(
      calendarRec.receiver,
      otherFields,
      undefined,
      calendarRec.dateFromFields
    );

    let { years, months } = ES.CalendarDateUntil(
      calendarRec.receiver,
      thisDate,
      otherDate,
      resolvedOptions,
  ...
  AddDate: (calendarRec, plainDate, duration, options = undefined) => {
    // dateAdd must be looked up if years, months, weeks != 0
    const years = GetSlot(duration, YEARS);
    const months = GetSlot(duration, MONTHS);
    const weeks = GetSlot(duration, WEEKS);
    if (years !== 0 || months !== 0 || weeks !== 0) {
      return ES.CalendarDateAdd(calendarRec.receiver, plainDate, duration, options, calendarRec.dateAdd);
    }
    ...
    return ES.CreateTemporalDate(year, month, day, calendarRec.receiver);
  },
  AddTime: (
    hour,
    minute,
  ...
  AddDuration: (
    ...
    if (!relativeTo) {
      ...
      const intermediate = ES.AddDate(calendarRec, relativeTo, dateDuration1);
      const end = ES.AddDate(calendarRec, intermediate, dateDuration2);

      const dateLargestUnit = ES.LargerOfTwoTemporalUnits('day', largestUnit);
      const differenceOptions = ObjectCreate(null);
      differenceOptions.largestUnit = dateLargestUnit;
      ({ years, months, weeks, days } = ES.DifferenceDate(calendarRec, relativeTo, end, differenceOptions));
      // Signs of date part and time part may not agree; balance them together
      ({ days, hours, minutes, seconds, milliseconds, microseconds, nanoseconds } = ES.BalanceDuration(
        days,
        bigInt(h1).add(h2),
  ...
  AddDateTime: (
    ...
    const TemporalDuration = GetIntrinsic('%Temporal.Duration%');
    const datePart = ES.CreateTemporalDate(year, month, day, calendarRec.receiver);
    const dateDuration = new TemporalDuration(years, months, weeks, days, 0, 0, 0, 0, 0, 0);
    const addedDate = ES.AddDate(calendarRec, datePart, dateDuration, options);
  ...
  AddZonedDateTime: (
    ...
    const dt = precalculatedDateTime ?? ES.GetPlainDateTimeFor(timeZoneRec, instant, calendarRec.receiver);
    if (ES.DurationSign(years, months, weeks, 0, 0, 0, 0, 0, 0, 0) === 0) {
      const overflow = ES.ToTemporalOverflow(options);
      const intermediate = ES.AddDaysToZonedDateTime(
        instant,
        dt,
        timeZoneRec,
        calendarRec.receiver,
    ...
    const dateDuration = new TemporalDuration(years, months, weeks, days, 0, 0, 0, 0, 0, 0);
    const addedDate = ES.CalendarDateAdd(calendarRec.receiver, datePart, dateDuration, options, calendarRec.dateAdd);
    ...
    const instantIntermediate = ES.GetInstantFor(timeZoneRec, dtIntermediate, 'compatible');
    return ES.AddInstant(GetSlot(instantIntermediate, EPOCHNANOSECONDS), h, min, s, ms, µs, ns);
  },
  AddDaysToZonedDateTime: (instant, dateTime, timeZoneRec, calendar, days, overflow = 'constrain') => {
    // getPossibleInstantsFor must be looked up
    ...
    const instantResult = ES.GetInstantFor(timeZoneRec, dateTimeResult, 'compatible');
    return {
  ...
  AddDurationToOrSubtractDurationFromPlainYearMonth: (operation, yearMonth, durationLike, options) => {
    ...
    // CAUTION: rebase conflict with #1315: different circumstances in which
    // dateAdd should be looked up when we no longer call daysInMonth
    const calendarRec = new MethodRecord(calendar);
    if (years !== 0 || months !== 0 || weeks !== 0 || days !== 0) {
      calendarRec.lookup('dateAdd');
    }
    const addedDate = ES.AddDate(calendarRec, startDate, durationToAdd, options);
  ...
  MoveRelativeDate: (calendarRec, relativeTo, duration) => {
    // dateAdd must be looked up if years, months, weeks != 0
    const later = ES.AddDate(calendarRec, relativeTo, duration);
    const days = ES.DaysUntil(relativeTo, later);
    return { relativeTo: later, days };
  },
  MoveRelativeZonedDateTime: (relativeTo, calendarRec, timeZoneRec, years, months, weeks, days) => {
    // getOffsetNanosecondsFor and getPossibleInstantsFor must be looked up
    ...
    return ES.CreateTemporalZonedDateTime(intermediateNs, timeZoneRec.receiver, calendarRec.receiver);
  },
  AdjustRoundedDurationDays: (
    ...
    const dayStartDateTime = ES.GetPlainDateTimeFor(timeZoneRec, dayStartInstant, calendar);
    const dayEnd = ES.AddDaysToZonedDateTime(
      dayStartInstant,
      dayStartDateTime,
      timeZoneRec,
      calendar,
  ...
  RoundDuration: (
    ...
    if (unit === 'year' || unit === 'month' || unit === 'week' || unit === 'day') {
      ...
      let intermediate;
      if (zonedRelativeTo) {
        intermediate = ES.MoveRelativeZonedDateTime(
          zonedRelativeTo,
          calendarRec,
          timeZoneRec,
          years,
      ...
      let deltaDays;
      ({ days: deltaDays, nanoseconds, dayLengthNs } = ES.NanosecondsToDays(nanoseconds, intermediate, timeZoneRec));
      days += deltaDays;
    ...
    switch (unit) {
      case 'year': {
        ...
        const yearsLater = ES.AddDate(calendarRec, plainRelativeTo, yearsDuration);
        const yearsMonthsWeeks = new TemporalDuration(years, months, weeks);
        const yearsMonthsWeeksLater = ES.AddDate(calendarRec, plainRelativeTo, yearsMonthsWeeks);
        ...
        const wholeDaysLater = ES.CreateTemporalDate(
          isoResult.year,
          isoResult.month,
          isoResult.day,
          calendarRec.receiver
        ...
        const yearsPassed = ES.DifferenceDate(calendarRec, plainRelativeTo, wholeDaysLater, untilOptions).years;
        years += yearsPassed;
        const oldRelativeTo = plainRelativeTo;
        const yearsPassedDuration = new TemporalDuration(yearsPassed);
        plainRelativeTo = ES.AddDate(calendarRec, plainRelativeTo, yearsPassedDuration);
        const daysPassed = ES.DaysUntil(oldRelativeTo, plainRelativeTo);
        days -= daysPassed;
        const oneYear = new TemporalDuration(days < 0 ? -1 : 1);
        let { days: oneYearDays } = ES.MoveRelativeDate(calendarRec, plainRelativeTo, oneYear);
      ...
      case 'month': {
        ...
        const yearsMonthsLater = ES.AddDate(calendarRec, plainRelativeTo, yearsMonths);
        const yearsMonthsWeeks = new TemporalDuration(years, months, weeks);
        const yearsMonthsWeeksLater = ES.AddDate(calendarRec, plainRelativeTo, yearsMonthsWeeks);
